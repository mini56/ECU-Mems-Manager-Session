...
RENDER_SCALE = 1.5
BLANK_THRESHOLD = 245
BLANK_FOREGROUND_MAX = 0.0001
NATIVE_RASTER_MIN_PAGE_COVERAGE = 0.35
...
def foreground_ratio(img):
    g=img.convert('L')
    if max(g.size)>1600: g.thumbnail((1600,1600))
    hist=g.histogram(); total=max(1,g.width*g.height)
    return sum(hist[:BLANK_THRESHOLD])/total

def is_effectively_blank(img): return foreground_ratio(img) <= BLANK_FOREGROUND_MAX

def native_bbox_to_page(b, rect, width, height):
    if width<=0 or height<=0 or rect.width<=0 or rect.height<=0: return None
    return box((rect.x0+b[0]*rect.width/width, rect.y0+b[1]*rect.height/height,
                rect.x0+b[2]*rect.width/width, rect.y0+b[3]*rect.height/height))

def render_bbox_to_page(b): return box((b[0]/RENDER_SCALE,b[1]/RENDER_SCALE,b[2]/RENDER_SCALE,b[3]/RENDER_SCALE))
...
def initdb(p):
    ...
    CREATE TABLE page(page_key TEXT PRIMARY KEY,document_key TEXT REFERENCES document,physical_page INT,page_class TEXT,native_text INT,ocr_used INT,ocr_source_kind TEXT,ocr_source_visual_key TEXT REFERENCES visual_asset,source_text_exact TEXT,source_text_sha256 TEXT,image_occurrence_count INT,vector_drawing_count INT,needs_review INT,review_reason TEXT,UNIQUE(document_key,physical_page));
    ...
    CREATE INDEX ixp ON page(document_key,physical_page); CREATE INDEX ixc ON content(page_key,order_index); CREATE INDEX ixo ON ocr_region(page_key,order_index); CREATE INDEX ixv ON visual_occurrence(page_key);''')
    return d
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--source-root',type=Path,required=True); ap.add_argument('--git-tree',type=Path,required=True); ap.add_argument('--source-commit',required=True); ap.add_argument('--out',type=Path,required=True); a=ap.parse_args()
    ...
    for di,(pdf,rel) in enumerate(sel,1):
        ...
        try:
            ...
            for pi in range(doc.page_count):
                p=doc[pi]; pn=pi+1; pk=f'{dk}_P{pn:04d}'; txt=p.get_text('text',sort=False); ls,bs=native(p) if txt.strip() else ([],[]); occ=0; occrows=[]; ocrrows=[]; raster_candidates=[]
                ...
                for im in p.get_images(full=True):
                    ...
                    if x not in assets:
                        try:
                            z=doc.extract_image(x); blob=z['image']; ext=(z.get('ext') or 'bin').lower(); vk=f'{dk}_X{x}'; rp=f'assets/{dk}/{vk}.{ext}'; (out/rp).write_bytes(blob)
                            assets[x]={'visual_key':vk,'path':rp,'width':int(z.get('width') or 0),'height':int(z.get('height') or 0)}
                            db.execute('INSERT OR IGNORE INTO visual_asset VALUES(?,?,?,?,?,?,?,?)',(vk,dk,x,ext,rp,shab(blob),assets[x]['width'],assets[x]['height']))
                    ...
                    for r in rects or [fitz.Rect(0,0,0,0)]:
                        occ+=1; occrows.append((f'{pk}_VIS{occ:03d}',pk,ai['visual_key'],json.dumps(box(r))))
                        coverage=max(0.0,float(r.width*r.height))/page_area
                        raster_candidates.append({'rect':r,'coverage':coverage,**ai})
                oc=0; rv=0; why=None; cl='blank'; src=txt or ''; ocr_kind=None; ocr_visual=None
                if txt.strip():
                    cl='mixed_native' if occ or vectors else 'text_native'
                elif occ or vectors:
                    render_rel=f'assets/{dk}/{pk}_NO_NATIVE_RENDER.png'
                    try:
                        pix=p.get_pixmap(matrix=fitz.Matrix(RENDER_SCALE,RENDER_SCALE),alpha=False); pix.save(str(out/render_rel)); render_img=Image.open(out/render_rel).convert('RGB')
                        if is_effectively_blank(render_img):
                            cl='blank'; cnt['blank_detected_render']+=1
                        else:
                            cl='visual_no_native_text'; oc=1; rv=1
                            candidate=max(raster_candidates,key=lambda c:c['coverage'],default=None)
                            if candidate and candidate['coverage']>=NATIVE_RASTER_MIN_PAGE_COVERAGE and candidate['width']>0 and candidate['height']>0:
                                ocr_kind='native_raster'; ocr_visual=candidate['visual_key']; ocr_img=Image.open(out/candidate['path']).convert('RGB'); regs=ocr(ocr_img)
                                for ri,x in enumerate(regs):
                                    pb=native_bbox_to_page(x['bbox'],candidate['rect'],candidate['width'],candidate['height'])
                                    if pb is not None: ocrrows.append((f'{pk}_OCR{ri+1:03d}',pk,ri,x['text'],json.dumps(pb),x['mean_confidence'],x['word_count']))
                            else:
                                ocr_kind='page_render'; regs=ocr(render_img)
                                for ri,x in enumerate(regs): ocrrows.append((f'{pk}_OCR{ri+1:03d}',pk,ri,x['text'],json.dumps(render_bbox_to_page(x['bbox'])),x['mean_confidence'],x['word_count']))
                            why=f'ocr_fallback_no_native_pdf_text_{ocr_kind}'
                            if ocrrows:
                                cl='visual_ocr'; src='\n'.join(x[3] for x in ocrrows)
                                cnt[f'ocr_{ocr_kind}_pages']+=1
                            else:
                                cl='visual_no_ocr_text'; reason=f'ocr_returned_no_regions_{ocr_kind}'; review.append({'page_key':pk,'document':repo,'physical_page':pn,'reason':reason})
                    except Exception as e:
                        cl='visual_no_native_text'; oc=1; rv=1; why=f'ocr_failed: {e}'; review.append({'page_key':pk,'document':repo,'physical_page':pn,'reason':why})
                db.execute('INSERT INTO page VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)',(pk,dk,pn,cl,1 if txt.strip() else 0,oc,ocr_kind,ocr_visual,src,shab(src.encode()) if src else None,occ,vectors,rv,why))
                db.executemany('INSERT INTO visual_occurrence VALUES(?,?,?,?)',occrows); db.executemany('INSERT INTO ocr_region VALUES(?,?,?,?,?,?,?)',ocrrows)
                if txt.strip():
                    lids=[]
    ...
    integ=db.execute('PRAGMA integrity_check').fetchone()[0]; fk=db.execute('PRAGMA foreign_key_check').fetchall(); dbc={t:db.execute(f'SELECT COUNT(*) FROM {t}').fetchone()[0] for t in ['document','page','line','content','visual_asset','visual_occurrence','ocr_region']}; db.close()
    pm=cnt['pages_accounted']==cnt['pages_expected_all_opened_documents']; clean=not err and not fatal_review; ok=clean and cnt['documents_processed']==cnt['documents_selected_english'] and pm and integ.lower()=='ok' and not fk
    jwrite(out/'document_audit.json',audit); jwrite(out/'needs_review.json',review)
    m={'method':'RAVEMEMS_FULL_CORPUS','source_commit':a.source_commit,'source_root':'main/rave','source_language_policy':'English source only; explicit non-English filename/path variants skipped','capture_scope':'ALL_RAVE_CONTENT_NO_ECU_FILTER','translation_performed_by_github':False,'localization_layer':'MEMS Manager','ocr_policy':'native PDF text first; prefer significant native raster when native text is absent; page render fallback for vector/mixed visual pages; render blank detection before OCR','ocr_bbox_space':'PDF_PAGE_POINTS','uncertainty_policy':'preserve and mark needs_review; never discard page data','counts':dict(cnt),'database_counts':dbc,'checks':{'all_selected_documents_opened':cnt['documents_processed']==cnt['documents_selected_english'],'all_pages_accounted':pm,'sqlite_integrity_ok':integ.lower()=='ok','sqlite_foreign_keys_ok':not fk,'no_processing_errors':clean,'no_execution_failures_in_needs_review':not fatal_review},'errors':err,'execution_failures_in_needs_review':fatal_review,'pass':ok}; jwrite(out/'manifest.json',m); print(json.dumps(m,indent=2))
