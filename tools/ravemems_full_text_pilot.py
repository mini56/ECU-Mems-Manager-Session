...
def structure(lines,blocks,visuals):
    L={x["line_id"]:x for x in lines}; B={b["block_index"]:b for b in blocks}; items=[]; refs=[]; sec="SAFETY INSTRUCTIONS"; sub=None
    def add(kind,src,bbox,ids,level=None):
        nonlocal items
        k=f"{DOC}_P{PAGE:03d}_I{len(items):03d}"
        it={"item_key":k,"order_index":len(items),"item_type":kind,"heading_level":level,"section":sec,"subsection":sub,"source_text":src,"source_line_ids":ids,"bbox":bbox,"applicability_status":"not_explicit_on_page","translation_required_for_display":kind!="printed_page_number"}
    ...
    for bi in order(blocks):
        ...
        if y>780 and len(ls)==2 and texts[0].strip().isdigit():
            add("printed_page_number",texts[0],ls[0]["bbox"],[ls[0]["line_id"]])
            add("running_footer_section",texts[1],ls[1]["bbox"],[ls[1]["line_id"]]); continue
        kind="instruction_paragraph"; level=None
        if y<60 and first=="GENERAL INFORMATION": kind="running_header"
        elif first=="SAFETY INSTRUCTIONS": kind="section_title"; level=1; sec=first; sub=None
        elif len(ls)==1 and allbold: kind="subsection_title"; level=2; sub=first
        elif first.startswith("WARNING:") and allbold: kind="warning"
        src=" ".join(x.strip() for x in texts if x.strip()); it=add(kind,src,box(b["bbox"]),[x["line_id"] for x in ls],level)
        for x in ls:
            for s in x["spans"]:
                exact=s["text"].strip(); key=exact.strip(" .,;:()[]")
                if key in REFS:
                    refs.append({"reference_key":f"{it['item_key']}_REF{len(refs)+1:02d}","item_key":it["item_key"],"source_text":key,"source_text_exact":exact,"type":"rave_section_cross_reference","bbox":s["bbox"]})
        if kind=="warning":
            cand=[]
            for v in visuals:
...
def dbwrite(path,source_sha,text,lines,items,visuals,refs):
    ...
    CREATE TABLE content(item_key TEXT PRIMARY KEY,page_key TEXT REFERENCES page,order_index INT,item_type TEXT,section_title TEXT,subsection_title TEXT,source_text TEXT,source_line_ids_json TEXT,bbox_json TEXT,applicability_status TEXT);
    ...
    for x in items: c.execute("INSERT INTO content VALUES(?,?,?,?,?,?,?,?,?,?)",(x["item_key"],pk,x["order_index"],x["item_type"],x.get("section"),x.get("subsection"),x["source_text"],json.dumps(x["source_line_ids"]),json.dumps(x["bbox"]),x["applicability_status"]))
...
def main():
    ...
    try:
        ...
        knowledge=[{"knowledge_key":x["item_key"],"domain":"rave_general_information","knowledge_type":x["item_type"],"topic":x.get("subsection") or x.get("section") or "GENERAL INFORMATION","source_key":DOC,"document":"MINI WORKSHOP MANUAL","source_language":"en","source_page":PAGE,"source_section":x.get("section"),"source_subsection":x.get("subsection"),"source_text":x["source_text"],"source_line_ids":x["source_line_ids"],"bbox":x["bbox"],"verification_level":"rave_source_exact","applicability":None,"applicability_status":"not_explicit_on_page"} for x in items]
