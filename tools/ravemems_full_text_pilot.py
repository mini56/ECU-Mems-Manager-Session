...
def dbwrite(path,source_sha,text,lines,items,visuals,refs):
    ...
    CREATE TABLE content_visual_relation(relation_key TEXT PRIMARY KEY,item_key TEXT NOT NULL REFERENCES content,visual_key TEXT NOT NULL REFERENCES visual,relation_type TEXT NOT NULL, UNIQUE(item_key,visual_key,relation_type));
    CREATE INDEX idx_content_visual_item ON content_visual_relation(item_key);
    CREATE INDEX idx_content_visual_visual ON content_visual_relation(visual_key);
    ...
    for x in items:
        if x.get("associated_visual_key"):
            c.execute("INSERT INTO content_visual_relation VALUES(?,?,?,?)",(f"{x['item_key']}_VISREL",x["item_key"],x["associated_visual_key"],"warning_pictogram"))
    for x in refs: c.execute("INSERT INTO crossref VALUES(?,?,?,?,?)",(x["reference_key"],x["item_key"],x["source_text"],x["type"],json.dumps(x["bbox"])))
    c.commit(); ok=c.execute("PRAGMA integrity_check").fetchone()[0]; fk=c.execute("PRAGMA foreign_key_check").fetchall(); c.close()
    if ok.lower()!="ok": raise SystemExit("SQLite integrity_check failed")
    if fk: raise SystemExit(f"SQLite foreign_key_check failed: {fk}")
