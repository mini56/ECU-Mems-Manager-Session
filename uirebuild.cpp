...
static QWidget *createRecorderTab(QTabWidget *tabs, QWidget *overview)
{
    ...
    QFrame *side=makeCard(page,QStringLiteral("recorderSideCard")); side->setMaximumWidth(310);
...
static QFrame *buildHeader(QMainWindow *window, QWidget *legacy)
{
    ...
    QWidget *commBox=new QWidget(header); QHBoxLayout *ch=new QHBoxLayout(commBox); ch->setContentsMargins(0,0,0,0); ch->setSpacing(5); comm->setParent(commBox); comm->setStyleSheet(QStringLiteral("color:#cbd3d9;font-weight:700;")); ch->addWidget(comm); if(good){good->setParent(commBox);good->setFixedSize(20,20);ch->addWidget(good);} if(bad){bad->setParent(commBox);bad->setFixedSize(20,20);ch->addWidget(bad);} h->addWidget(commBox);
    connectButton->setParent(header); disconnectButton->setParent(header); connectButton->setMinimumWidth(92); disconnectButton->setMinimumWidth(102); h->addWidget(connectButton); h->addWidget(disconnectButton);
    legacy->hide(); legacy->setMaximumSize(0,0); return header;
}

static QFrame *buildBottomStatus(QMainWindow *window)
{
    QFrame *bar=new QFrame(window->centralWidget()); bar->setObjectName(QStringLiteral("uiRebuildStatus")); bar->setAttribute(Qt::WA_StyledBackground,true); bar->setFixedHeight(36);
