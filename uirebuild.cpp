...
static const char *kPanel2 = "#0d1319";
...
static QFrame *makeCard(QWidget *parent, const QString &objectName = QStringLiteral("uiRebuildCard"))
{
    QFrame *card = new QFrame(parent);
    card->setObjectName(objectName);
    card->setAttribute(Qt::WA_StyledBackground, true);
    card->setStyleSheet(QStringLiteral(
        "#%1{background:%2;border:1px solid %3;border-radius:5px;}"
...
{
    QFrame *hero = makeCard(parent, QStringLiteral("uiRebuildHero"));
    ...
    QFont sf = s->font();
    sf.setPointSizeF(7.6);
    sf.setLetterSpacing(QFont::AbsoluteSpacing, 0.45);
    s->setFont(sf);
...
static void prepareViewWidgets(QWidget *page)
{
    ...
    for (QTableView *v : views) {
        ...
        if (v->horizontalHeader()) {
            ...
            v->horizontalHeader()->setDefaultAlignment(Qt::AlignLeft | Qt::AlignVCenter);
        }
...
static void composeGenericPage(QWidget *page, const QString &title)
{
    ...
    for (QWidget *block : blocks) {
        ...
        QFrame *card = makeCard(page, QStringLiteral("uiRebuildCard_%1_%2").arg(row).arg(col));
        ...
        block->setStyleSheet(QStringLiteral("background:transparent;border:0;"));
    ...
    if (blocks.isEmpty()) {
        QFrame *card = makeCard(page,QStringLiteral("uiRebuildEmptyCard"));
        QVBoxLayout *v = new QVBoxLayout(card);
        v->setContentsMargins(14,14,14,14);
        QLabel *label = new QLabel(QStringLiteral("Contenu ECU"),card);
        label->setStyleSheet(QStringLiteral("color:%1;background:transparent;").arg(QString::fromLatin1(kMuted)));
        v->addWidget(label,0,Qt::AlignTop|Qt::AlignLeft);
        v->addStretch(1);
...
static QWidget *createRecorderTab(QTabWidget *tabs, QWidget *overview)
{
    ...
    QFrame *line = new QFrame(mainCard); line->setFrameShape(QFrame::HLine); line->setStyleSheet(QStringLiteral("background:%1;border:0;max-height:1px;").arg(QString::fromLatin1(kBorder)));
    mainLayout->addWidget(line);
    loggerBox->setParent(mainCard);
    loggerBox->setMinimumWidth(0);
    loggerBox->setMaximumWidth(QWIDGETSIZE_MAX);
    loggerBox->setSizePolicy(QSizePolicy::Expanding,QSizePolicy::Fixed);
    mainLayout->addWidget(loggerBox,0);
    mainLayout->addStretch(1);
    body->addWidget(mainCard,3);

    QFrame *side = makeCard(page,QStringLiteral("recorderSideCard"));
    side->setMaximumWidth(310);
    QVBoxLayout *sv = new QVBoxLayout(side);
    sv->setContentsMargins(16,16,16,16);
    QLabel *st = new QLabel(QStringLiteral("ENREGISTREMENT"),side);
    QFont stf=st->font(); stf.setBold(true); stf.setPointSizeF(10); st->setFont(stf);
    st->setStyleSheet(QStringLiteral("color:%1;background:transparent;").arg(QString::fromLatin1(kOrange2)));
    QLabel *desc = new QLabel(QStringLiteral("Nom de fichier, démarrage et arrêt de la session. Les commandes restent reliées à l'enregistreur réel du programme."),side);
    desc->setWordWrap(true);
    desc->setStyleSheet(QStringLiteral("color:%1;background:transparent;").arg(QString::fromLatin1(kMuted)));
    sv->addWidget(st);
    sv->addWidget(desc);
    sv->addStretch(1);
...
static QFrame *buildHeader(QMainWindow *window, QWidget *legacy)
{
    ...
    header->setObjectName(QStringLiteral("uiRebuildHeader"));
    header->setAttribute(Qt::WA_StyledBackground,true);
    header->setFixedHeight(56);
    header->setStyleSheet(QStringLiteral(
        "#uiRebuildHeader{background:#080d12;border:0;border-bottom:1px solid %1;}"
        "#uiRebuildHeader QLabel{background:transparent;border:0;}"
        "#uiRebuildHeader QPushButton{min-height:28px;}"
        "#m_connectButton{background:%2;color:#11161a;border:1px solid %3;font-weight:800;}"
        "#m_disconnectButton{background:#b52323;color:white;border:1px solid #d83a3a;font-weight:800;}"
        "#m_connectButton:disabled,#m_disconnectButton:disabled{background:#171e24;color:#5f6972;border-color:#29323a;}"
    ).arg(QString::fromLatin1(kBorder),QString::fromLatin1(kOrange),QString::fromLatin1(kOrange2)));
    ...
    if (!p.isNull()) logo->setPixmap(p.scaled(34,34,Qt::KeepAspectRatio,Qt::SmoothTransformation));
    logo->setFixedSize(38,38);
    logo->setAlignment(Qt::AlignCenter);
    h->addWidget(logo);

    QVBoxLayout *brandV=new QVBoxLayout;
    brandV->setContentsMargins(0,0,0,0);
    brandV->setSpacing(0);
    QLabel *brand=new QLabel(QStringLiteral("ECU MEMS MANAGER"),header);
    QFont bf=brand->font(); bf.setBold(true); bf.setPointSizeF(10.5); brand->setFont(bf);
    brand->setStyleSheet(QStringLiteral("color:#f5f7f8;letter-spacing:.5px;"));
    QLabel *ver=new QLabel(QStringLiteral("v%1.%2.%3").arg(VER_MAJOR).arg(VER_MINOR).arg(VER_PATCH),header);
    ver->setStyleSheet(QStringLiteral("color:%1;font-weight:700;").arg(QString::fromLatin1(kOrange2)));
    brandV->addWidget(brand);
    brandV->addWidget(ver);
    h->addLayout(brandV);
    ...
    ecu->setParent(header);
    ecu->setStyleSheet(QStringLiteral("color:#dfe4e8;font-weight:700;padding:0 8px;"));
    h->addWidget(ecu);

    QSettings s(QSettings::IniFormat,QSettings::UserScope,PROJECTNAME);
    s.beginGroup(QStringLiteral("Settings"));
    QString port=s.value(QStringLiteral("SerialDevice"),QStringLiteral("--")).toString();
    s.endGroup();
    if (port.trimmed().isEmpty()) port=QStringLiteral("--");
    ...
    QLabel *freqLabel=new QLabel(QStringLiteral("Fréquence\nauto"),header); freqLabel->setAlignment(Qt::AlignCenter); freqLabel->setStyleSheet(QStringLiteral("color:#cbd3d9;font-weight:600;")); h->addWidget(freqLabel);
    h->addStretch(1);

    QWidget *commBox=new QWidget(header);
    QHBoxLayout *ch=new QHBoxLayout(commBox); ch->setContentsMargins(0,0,0,0); ch->setSpacing(5);
    comm->setParent(commBox); comm->setStyleSheet(QStringLiteral("color:#cbd3d9;font-weight:700;")); ch->addWidget(comm);
    if (good) { good->setParent(commBox); good->setFixedSize(20,20); ch->addWidget(good); }
    if (bad) { bad->setParent(commBox); bad->setFixedSize(20,20); ch->addWidget(bad); }
    h->addWidget(commBox);

    connectButton->setParent(header); disconnectButton->setParent(header);
    connectButton->setMinimumWidth(92); disconnectButton->setMinimumWidth(102);
    h->addWidget(connectButton); h->addWidget(disconnectButton);

    legacy->hide(); legacy->setMaximumSize(0,0);
    return header;
}
...
static QFrame *buildBottomStatus(QMainWindow *window)
{
    ...
    bar->setStyleSheet(QStringLiteral("#uiRebuildStatus{background:#080d12;border-top:1px solid %1;} #uiRebuildStatus QLabel{background:transparent;border-right:1px solid %1;color:#c9d1d7;padding:0 12px;} #uiRebuildStatus QPushButton{margin:4px 6px;padding:2px 12px;}").arg(QString::fromLatin1(kBorder)));
    QHBoxLayout *h=new QHBoxLayout(bar); h->setContentsMargins(0,0,0,0); h->setSpacing(0);

    QLabel *file=new QLabel(QStringLiteral("Fichier : --"),bar);
    QLabel *loop=new QLabel(QStringLiteral("Boucle : --"),bar);
    QLabel *lambda=new QLabel(QStringLiteral("Lambda : --"),bar);
    QLabel *system=new QLabel(QStringLiteral("Système : --"),bar); system->setStyleSheet(QStringLiteral("color:#65db79;background:transparent;border-right:1px solid %1;padding:0 12px;").arg(QString::fromLatin1(kBorder)));
    QLabel *inject=new QLabel(QStringLiteral("Injection : -- ms"),bar);
    QLabel *air=new QLabel(QStringLiteral("Air : -- °C"),bar);
    h->addWidget(file,2); h->addWidget(loop,1); h->addWidget(lambda,1); h->addWidget(system,1); h->addWidget(inject,1); h->addWidget(air,1);
    QPushButton *capture=new QPushButton(QStringLiteral("Capture écran"),bar); h->addWidget(capture,1);
    ...
    QTimer *timer=new QTimer(bar); timer->setInterval(400);
    QObject::connect(timer,&QTimer::timeout,bar,[=](){
        if (QLineEdit *f=window->findChild<QLineEdit*>(QStringLiteral("m_logFileNameBox"))) file->setText(QStringLiteral("Fichier : %1").arg(f->text()));
        if (QWidget *w=window->findChild<QWidget*>(QStringLiteral("m_closedLoopLed"))) loop->setText(QStringLiteral("Boucle : %1").arg(w->property("checked").toBool()?QStringLiteral("fermée"):QStringLiteral("ouverte")));
        if (QWidget *w=window->findChild<QWidget*>(QStringLiteral("m_lambda_voltage"))) lambda->setText(QStringLiteral("Lambda : %1").arg(w->property("value").toString()));
        if (QWidget *w=window->findChild<QWidget*>(QStringLiteral("m_engine_error"))) system->setText(w->property("checked").toBool()?QStringLiteral("Système : défaut"):QStringLiteral("Système : OK"));
        if (QWidget *w=window->findChild<QWidget*>(QStringLiteral("m_airTempGauge"))) air->setText(QStringLiteral("Air : %1").arg(w->property("value").toString()));
    });
    timer->start();
    return bar;
}
...
{
public:
    explicit UiRebuildInstaller(QObject *parent=nullptr):QObject(parent){}
protected:
    bool eventFilter(QObject *watched,QEvent *event) override
    {
        if ((event->type()!=QEvent::Show && event->type()!=QEvent::Polish) || !watched) return QObject::eventFilter(watched,event);
        QMainWindow *window=qobject_cast<QMainWindow*>(watched);
        if (!window || window->objectName()!=QStringLiteral("MainWindow") || window->property("uiRebuildInstalled").toBool()) return QObject::eventFilter(watched,event);
        ...
        QWidget *legacy=central?central->findChild<QWidget*>(QStringLiteral("layoutWidget_7")):nullptr;
        if (!tabs || !root || !legacy) return QObject::eventFilter(watched,event);
        window->setProperty("uiRebuildInstalled",true);
        QTimer::singleShot(0,window,[=](){ rebuild(window,central,root,tabs,legacy); });
...
private:
    static void rebuild(QMainWindow *window,QWidget *central,QVBoxLayout *root,QTabWidget *tabs,QWidget *legacy)
    {
        ...
        central->setStyleSheet(QStringLiteral("background:%1;").arg(QString::fromLatin1(kBg)));

        QFrame *header=buildHeader(window,legacy);
        if (header) root->insertWidget(0,header);

        QWidget *overview=nullptr;
        for (int i=0;i<tabs->count();++i) {
            QWidget *p=realPage(tabs->widget(i));
            if (p && p->objectName()==QStringLiteral("overview_tab")) { overview=p; break; }
        }
        if (overview && tabs->findChild<QWidget*>(QStringLiteral("recorder_tab"))==nullptr) {
            if (QWidget *rec=createRecorderTab(tabs,overview)) tabs->addTab(rec,QStringLiteral("Enregistreur"));
        }

        const int oldIndex=root->indexOf(tabs);
        if (oldIndex>=0) root->removeWidget(tabs);
        QFrame *workspace=new QFrame(central);
        workspace->setObjectName(QStringLiteral("uiRebuildWorkspace"));
        workspace->setAttribute(Qt::WA_StyledBackground,true);
        workspace->setStyleSheet(QStringLiteral("#uiRebuildWorkspace{background:%1;border:0;}").arg(QString::fromLatin1(kBg)));
        QHBoxLayout *wh=new QHBoxLayout(workspace); wh->setContentsMargins(0,0,0,0); wh->setSpacing(0);

        QListWidget *nav=new QListWidget(workspace);
        nav->setObjectName(QStringLiteral("uiRebuildNav"));
        ...
        nav->setStyleSheet(QStringLiteral(
            "#uiRebuildNav{background:#0d1318;color:#c7d0d6;border:0;border-right:1px solid %1;padding:8px 0;}"
            "#uiRebuildNav::item{min-height:35px;padding:2px 14px;border-left:3px solid transparent;font-weight:650;}"
            "#uiRebuildNav::item:hover{background:#161d23;color:white;}"
            "#uiRebuildNav::item:selected{background:#1c211f;color:%2;border-left:3px solid %3;}"
        ).arg(QString::fromLatin1(kBorder),QString::fromLatin1(kOrange2),QString::fromLatin1(kOrange)));
        ...
        wh->addWidget(nav); wh->addWidget(tabs,1);
        root->insertWidget(oldIndex<0?1:oldIndex,workspace,1);
        ...
        for (int i=0;i<tabs->count();++i) {
            ...
            if (!page) continue;
            if (QScrollArea *scroll=qobject_cast<QScrollArea*>(tabs->widget(i))) {
                scroll->setWidgetResizable(true);
                scroll->setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
                scroll->setVerticalScrollBarPolicy(Qt::ScrollBarAsNeeded);
                page->setMinimumSize(0,0);
            }
            const QString title=titleForIndex(tabs,i);
            if (page->objectName()!=QStringLiteral("overview_tab") && page->objectName()!=QStringLiteral("recorder_tab") &&
                QString::fromLatin1(page->metaObject()->className())!=QStringLiteral("AnalysisTab")) {
                composeGenericPage(page,title);
            }
        }

        QFrame *status=buildBottomStatus(window);
        root->addWidget(status);
        tabs->setCurrentIndex(0);
    }
};

void installUiRebuild()
{
    QApplication *app=qobject_cast<QApplication*>(QCoreApplication::instance());
    if (!app) return;
    UiRebuildInstaller *installer=new UiRebuildInstaller(app);
