...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evidence-based GENERIC visual validation")
    parser.add_argument("--pdf", type=Path, required=True)
    parser.add_argument("--out", type=Path, required=True)
    args = parser.parse_args()

    db_path = args.out / "ravemems_v2.sqlite"
    ...
    try:
        ...
        print("GENERIC_VISUAL_FIDELITY_VERIFIED", len(fidelity_ok))
        print("GENERIC_VISUAL_FIDELITY_FAILED", len(fidelity_failed))
        print("GENERIC_VISUAL_LINKS_VERIFIED", len(links_ok))
        print("GENERIC_VISUAL_LINKS_FAILED", len(links_failed))
        print("GENERIC_POST_VISUAL_AUDIT_ISSUES", len(issues))
        print("GENERIC_POST_VISUAL_SQLITE_INTEGRITY", integrity)
        print("GENERIC_POST_VISUAL_FOREIGN_KEYS", len(foreign_keys))

        if fidelity_failed or links_failed or str(integrity).lower() != "ok" or foreign_keys or issues:
            return 1
        print("RAVEMEMS_V2_GENERIC_VISUAL_VALIDATION_PASS")
        return 0
    finally:
        db.close()
        doc.close()
