...
static void emptyLayout(QLayout *layout)
{
    ...
    while(QLayoutItem *it=layout->takeAt(0)){
        if(it->layout()) emptyLayout(it->layout());
        delete it;
    }
}
...
static QFrame *hero(QWidget *parent,const QString &title,const QString &subtitle)
{
    ...
    QVBoxLayout *v=new QVBoxLayout(f); v->setContentsMargins(14,7,14,7); v->setSpacing(0);
    QLabel *t=new QLabel(title.toUpper(),f); QFont tf=t->font(); tf.setBold(true); tf.setPointSizeF(9.4); t->setFont(tf); t->setStyleSheet(QStringLiteral("color:#ff9828;background:transparent;border:0;"));
    QLabel *s=new QLabel(subtitle,f); QFont sf=s->font(); sf.setPointSizeF(7.0); s->setFont(sf); s->setStyleSheet(QStringLiteral("color:#8d99a3;background:transparent;border:0;"));
    v->addWidget(t); v->addWidget(s); f->setMaximumHeight(48); return f;
}
...
static QFrame *card(QWidget *parent,const QString &title)
{
    ...
    v->addWidget(t);
    QFrame *line=new QFrame(f); line->setFrameShape(QFrame::HLine); line->setStyleSheet(QStringLiteral("background:#29343e;border:0;max-height:1px;")); v->addWidget(line);
    return f;
}
...
static void styleRawBlock(QWidget *block)
{
    ...
    for(QLabel *l:block->findChildren<QLabel*>()){
        ...
        l->setStyleSheet(head?QStringLiteral("color:#ff9828;background:transparent;font-weight:700;"):QStringLiteral("color:#dce3e8;background:transparent;"));
    }
}
...
static void rebuildRaw(QTabWidget *tabs)
{
    ...
    for(int i=0;i<tabs->count();++i){QWidget *p=realPage(tabs->widget(i));if(p&&p->objectName()==QStringLiteral("raw")){page=p;break;}}
    ...
    QFrame *cb=card(page,QStringLiteral("TRAME 80")); static_cast<QVBoxLayout*>(cb->layout())->addWidget(b,1);
    body->addWidget(ca,1); body->addWidget(cb,1); root->addLayout(body,1);
}
...
{
    QWidget *page=nullptr;
    for(int i=0;i<tabs->count();++i){QWidget *p=realPage(tabs->widget(i));if(p&&p->objectName()==QStringLiteral("ECU")){page=p;break;}}
    if(!page||page->property("strictInteractiveBuilt").toBool()) return;
    page->setProperty("strictInteractiveBuilt",true); cleanupOldShell(page);
    QLabel *l23=page->findChild<QLabel*>(QStringLiteral("label_23")); QLabel *l24=page->findChild<QLabel*>(QStringLiteral("label_24")); QLabel *l48=page->findChild<QLabel*>(QStringLiteral("label_48")); QLabel *l49=page->findChild<QLabel*>(QStringLiteral("label_49")); QLabel *l50=page->findChild<QLabel*>(QStringLiteral("label_50")); QLabel *l51=page->findChild<QLabel*>(QStringLiteral("label_51")); QLabel *l22=page->findChild<QLabel*>(QStringLiteral("label_22"));
    QPushButton *button=page->findChild<QPushButton*>(QStringLiteral("interactive_push_button")); QLineEdit *info=page->findChild<QLineEdit*>(QStringLiteral("lineEdit_2")); QLabel *empty=page->findChild<QLabel*>(QStringLiteral("label_9")); if(empty)empty->hide();
    QVBoxLayout *root=nullptr; replaceRoot(page,root); root->addWidget(hero(page,QStringLiteral("MODE INTERACTIF"),QStringLiteral("COMMANDES MANUELLES ET MODE TECHNIQUE")));
    QHBoxLayout *body=new QHBoxLayout; body->setSpacing(8);
    QFrame *instructions=card(page,QStringLiteral("UTILISATION")); QVBoxLayout *iv=static_cast<QVBoxLayout*>(instructions->layout()); moveLabel(l23,instructions,iv);moveLabel(l24,instructions,iv);moveLabel(l48,instructions,iv);moveLabel(l49,instructions,iv);moveLabel(l50,instructions,iv);
    if(button){button->setParent(instructions);button->setMinimumHeight(32);button->setMaximumWidth(220);button->show();iv->addWidget(button,0,Qt::AlignLeft);} iv->addStretch(1);
    QFrame *safety=card(page,QStringLiteral("SÉCURITÉ")); QVBoxLayout *sv=static_cast<QVBoxLayout*>(safety->layout()); moveLabel(l51,safety,sv,true);moveLabel(l22,safety,sv,true);
    if(info){info->setParent(safety);info->setReadOnly(true);info->setMinimumHeight(32);info->show();sv->addWidget(info);}sv->addStretch(1);
    body->addWidget(instructions,3);body->addWidget(safety,2);root->addLayout(body,1);
}
...
void installDedicatedPages(){QApplication *a=qobject_cast<QApplication*>(QCoreApplication::instance());if(a)a->installEventFilter(new DedicatedInstaller(a));}
