...
**Statut : VALIDÉ PAR L’UTILISATEUR — VERSION FIGÉE AVANT IMPLÉMENTATION**  
...
Si plusieurs réponses incompatibles restent possibles, l’IA doit demander uniquement le discriminant réellement utile, en utilisant d’abord les variantes qui correspondent au véhicule ou au contexte déjà identifié par la recherche. **Exemple : si les résultats pertinents concernent une Mini et distinguent Mini SPi et Mini MPi, l’IA demande d’abord : « Est-ce pour une Mini SPi ou une Mini MPi ? », tout en laissant la possibilité de répondre qu’il s’agit d’un autre véhicule.** SPi / MPi ne doit toutefois jamais être considéré comme propre à la Mini : d’autres modèles et véhicules peuvent utiliser ces architectures. Pour un autre véhicule, l’IA doit donc employer les variantes réellement présentes pour ce véhicule et demander les informations complémentaires nécessaires seulement si elles manquent. Si un véhicule / ECU est connecté, la règle précédente reste prioritaire : l’IA confirme d’abord que la question concerne bien ce véhicule avant d’utiliser son contexte.
...
Lorsqu’il existe plusieurs **éditions ou révisions d’un même manuel**, chaque information reste rattachée à son édition, sa révision et son contexte d’applicabilité. Une édition plus récente ne remplace jamais automatiquement une information plus ancienne pour tous les véhicules.
...
Un **visuel techniquement dégradé ou incorrect n’est pas un résultat acceptable**. Tout doit être mis en œuvre pour l’extraire proprement et fidèlement depuis le document original. Si un cas résiste, il reste un défaut bloquant `à corriger` / `à vérifier` et ne doit pas être accepté comme visuel final.
...
Si une traduction dans la langue active n’est pas encore validée, l’information source reste disponible. Une **traduction provisoire** peut être utilisée pour ne pas perdre l’information, mais elle reste identifiée comme non validée et enregistrée `à vérifier`. L’audit doit la contrôler avant validation finale du corpus.
...
## 41. RELECTURE UTILISATEUR TERMINÉE
...
# FIN DU CAHIER DES CHARGES VALIDÉ

**Version figée après relecture et validation explicite de l’utilisateur le 2 septembre 2026. Toute modification fonctionnelle ultérieure devra être explicitement validée avant implémentation.**
